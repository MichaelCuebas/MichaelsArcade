...
document
  ...
  .setAttribute("src", "movingPurpleBackground.mp4");
...
function squareResetFunc(a, z, wide, tall, color) {
  for (a; a < z; a++) {
    if (squares[a].yPos > 580) {
      if (a < z)
        squares[a] = new square(
          arrayRandomChoice,
          -200,
          squareThickness + wide,
          squareHeight + tall,
          color
        );
    }
...
function collisionDetection2(i) {
  if (collision === true) {
    if (
      squareX + (squareThickness - 1) >= circles[i].xPos &&
      squareX <= circles[i].xPos + (circles[i].radius * 2 - 1) &&
      squareY + squareHeight >= circles[i].yPos + 1 &&
      squareY + 1 <= circles[i].yPos + circles[i].radius * 2 &&
      squareColor !== circles[i].color
    ) {
      return (showingEndScreen = true);
    }
  }
...
function collisionDetection3(i) {
  if (collision === true) {
    if (
      squareX + (squareThickness - 1) >= circles2[i].xPos &&
      squareX <= circles2[i].xPos + (circles2[i].radius * 2 - 1) &&
      squareY + squareHeight >= circles2[i].yPos + 1 &&
      squareY + 1 <= circles2[i].yPos + circles2[i].radius * 2 &&
      squareColor !== circles2[i].color
    ) {
      return (showingEndScreen = true);
    }
  }
...
function handleMouseClickStart() {
  if (showingStartScreen) {
    ...
    document
      .querySelector("#myVideo")
      .setAttribute("src", "droppingSquares.mp4");
  }
}
...
class square {
  constructor(xPos, yPos, width, height, color) {
    if (arrayRandomChoice === arrayRandomFull) {
      ...
      this.xPos =
        arrayRandomRight[Math.floor(Math.random() * arrayRandomRight.length)];
    } else if (arrayRandomChoice === arrayRandomLeftX) {
      this.xPos =
        arrayRandomLeft[Math.floor(Math.random() * arrayRandomLeft.length)];
    } else {
      this.xPos = xPos;
    }
    this.yPos = yPos;
...
function drawEverything() {
  ...
  if (showingStartScreen) {
    ...
    canvasContext.fillText(
      "Use 'W' 'A' 'S' 'D' to avoid the falling squares",
      90,
      400
    );
    canvasContext.font = "bold 20px Helvetica, Arial, sans-serif";
    canvasContext.fillText(
      "Falling squares of the same color can be passed through safely",
      110,
      450
    );
    seconds = -1;
